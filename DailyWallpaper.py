...
def create_weather_overlay():
    ...
    for row in range(8):
        ...
        pilDrawImage.text((colSize, headerHeight + rowSize * row), forecastsDf.loc[row, "temp"], font=trueFont, anchor="ls")
        pilDrawImage.text((colSize * 2 - 20, headerHeight + rowSize * row), forecastsDf.loc[row, "desc"], font=trueFont, anchor="ls")
        pilDrawImage.text((colSize * 3 + 40, headerHeight + rowSize * row), forecastsDf.loc[row, "perc"], font=trueFont, anchor="ls")
...
infoPath = os.path.join(os.path.join(os.path.join(os.path.join(os.environ['USERPROFILE']),'OneDrive'),'Desktop'),'BackgroundInfo.txt')
f = open(infoPath,"w")
